...
Executa o pipeline completo do projeto, na ordem correta, do zero:
  ...
  2) Roda a busca empírica ampla de arquitetura x taxa de aprendizado (gera results/logs/baseline_search_log.csv).
  3) Roda a busca de intensidade de cada técnica de ablação (gera results/logs/ablation_hparam_search_log.csv).
  4) Treina os 5 modelos finais (baseline + 4 ablações), gera todos os gráficos e a tabela comparativa final de métricas.
